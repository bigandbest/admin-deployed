...
const API_URL = `${import.meta.env.VITE_API_BASE_URL}/quick-pick-group`;
const QUICK_PICK_API_URL = `${import.meta.env.VITE_API_BASE_URL}/quick-pick`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (initialData) {
        ...
        notifications.show({
          color: "green",
          message: "Quick Pick Group updated successfully.",
        });
      } else {
        // Add new quick pick
        ...
        notifications.show({
          color: "green",
          message: "Quick Pick Group added successfully.",
        });
      }
      ...
      notifications.show({
        color: "red",
        message: "Failed to save quick pick group.",
      });
    }
  };
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center">
      <div className="bg-white p-8 rounded-md shadow-lg w-96">
        <h2 className="text-2xl font-bold mb-4">
          {initialData ? "Edit Quick Pick Group" : "Add Quick Pick Group"}
        </h2>
...
const QuickPickGroupPage = () => {
  ...
  const fetchQuickPickGroups = async () => {
    try {
      ...
      notifications.show({
        color: "red",
        message: "Failed to load quick pick groups.",
      });
    }
  };
    ...
    if (quickPickId) {
      try {
        const response = await axios.get(
          `${QUICK_PICK_API_URL}/${quickPickId}`
        );
        ...
        notifications.show({
          color: "red",
          message: "Failed to load quick pick name.",
        });
      }
    }
  };
  ...
  const handleDelete = async (id) => {
    if (
      window.confirm("Are you sure you want to delete this quick pick group?")
    ) {
      try {
        ...
        notifications.show({
          color: "green",
          message: "Quick Pick Group deleted successfully.",
        });
      } catch (error) {
        console.error("Error deleting quick pick group:", error);
        notifications.show({
          color: "red",
          message: "Failed to delete quick pick group.",
        });
      }
    }
  };
                  ...
                  <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                    <div className="flex space-x-2">
                      ...
                      <button
                        ...
                        onClick={() =>
                          navigate(`/quick-pick-group/products/${group.id}`)
                        }
                      >
                        Products
                      </button>
