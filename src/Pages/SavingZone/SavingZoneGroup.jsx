...
const API_URL = `${import.meta.env.VITE_API_BASE_URL}/saving-zone-group`;
const SAVING_ZONE_API_URL = `${import.meta.env.VITE_API_BASE_URL}/saving-zone`;

// Component to handle adding/editing a Saving Zone Group
const SavingZoneGroupForm = ({
  initialData,
  onSave,
  onCancel,
  savingZoneId,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (initialData) {
        ...
        notifications.show({
          color: "green",
          message: "Saving Zone Group updated successfully.",
        });
      } else {
        ...
        notifications.show({
          color: "green",
          message: "Saving Zone Group added successfully.",
        });
      }
      ...
      notifications.show({
        color: "red",
        message: "Failed to save saving zone group.",
      });
    }
  };
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center">
      <div className="bg-white p-8 rounded-md shadow-lg w-96">
        <h2 className="text-2xl font-bold mb-4">
          {initialData ? "Edit Saving Zone Group" : "Add Saving Zone Group"}
        </h2>
...
const SavingZoneGroupPage = () => {
  ...
  const fetchSavingZoneGroups = async () => {
    try {
      ...
      notifications.show({
        color: "red",
        message: "Failed to load saving zone groups.",
      });
    }
  };
    ...
    if (savingZoneId) {
      try {
        const response = await axios.get(
          `${SAVING_ZONE_API_URL}/${savingZoneId}`
        );
        ...
        notifications.show({
          color: "red",
          message: "Failed to load saving zone name.",
        });
      }
    }
  };
  ...
  const handleDelete = async (id) => {
    if (
      window.confirm("Are you sure you want to delete this saving zone group?")
    ) {
      try {
        ...
        notifications.show({
          color: "green",
          message: "Saving Zone Group deleted successfully.",
        });
      } catch (error) {
        console.error("Error deleting saving zone group:", error);
        notifications.show({
          color: "red",
          message: "Failed to delete saving zone group.",
        });
      }
    }
  };
                  ...
                  <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                    <div className="flex space-x-2">
                      ...
                      <button
                        ...
                        onClick={() =>
                          navigate(`/saving-zone-group/products/${group.id}`)
                        }
                      >
                        Products
                      </button>
