...
const AddBannerGroupProducts = () => {
  ...
  const fetchGroupData = useCallback(async () => {
    try {
      ...
      const groupResult = await getBannerGroupById(id);
      if (groupResult.success) {
        setGroup(groupResult.bannerGroup);
      } else {
        throw new Error(groupResult.error);
      }

      // Fetch the products associated with this Banner Group
      const productsResult = await getProductsForBannerGroup(id);
  ...
  const handleRemoveProduct = async (productId) => {
    ...
    if (isConfirmed) {
      try {
        const result = await removeProductFromBannerGroup(productId, id);
        if (result.success) {
          notifications.show({
            color: "green",
  ...
  const handleAddProduct = async () => {
    ...
    try {
      const result = await mapProductToBannerGroup(selectedProduct, id);
      if (result.success) {
        notifications.show({
          color: "green",
  ...
  return (
    <div className="p-8 bg-gray-100 min-h-screen">
      <button
        ...
        onClick={() =>
          navigate(`/add-banner-group?bannerId=${group.banner_id}`)
        }
      >
        ← Back to the Groups
      </button>
      ...
      <h1 className="text-3xl font-bold mb-2">
        Manage Products for the Banner Group: {group.name}
      </h1>
