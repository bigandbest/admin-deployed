...
const DailyDealForm = ({ initialData, onSave, onCancel }) => {
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      try {
        const response = await api.get("/add-banner/type/daily_deals");
        if (response.data.success) {
          const activeBanners = response.data.banners.filter((b) => b.active);
          setAvailableBanners(activeBanners);
                            ...
                            <div className="flex items-start justify-between">
                              ...
                              <button
                                ...
                                onClick={() => {
                                  if (
                                    window.confirm(
                                      `Remove "${item.products?.name || "this product"
                                      }" from this deal?`
                                    )
                                  ) {
